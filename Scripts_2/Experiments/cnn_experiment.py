# This script implements the framework for training and evaluating
# convolutional neural networks (CNNs) for technical analysis.
# It replicates the methodology described in Jiang, Kelly, and Xiu (2023).
# The Experiment class and its utilities manage data preparation, training,
# validation, evaluation, and portfolio construction workflows.
...
# Experiment Class:
#
# Encapsulates the full CNN workflow, including:
# - Initialization of experiment parameters (window size, country, etc.).
# - DataLoader preparation.
# - Model training (single & ensemble), evaluation, and checkpointing.
# - Portfolio construction from ensemble predictions.

class Experiment(object):

    # __init__:
    #
    # Initializes the experiment with user-specified hyperparameters:
    # - ws: window size for chart input.
    # - cnn: CNN model class to instantiate.
    # - country: target market/country.
    # - Various flags: MA, VB, OHLC chart formats, etc.
    # - Training configuration: dropout, epochs, optimizer params.
    # - Sets up internal paths for saving models, logs, and ensemble results.
    # - Supports transfer learning by specifying pre-trained sources.
    ...
    def __init__(
        ...
        pf_freq=None,
        tensorboard=False,
        weight_decay=0, # L2 penalty for optimizer
        ...
        assert transfer_learning in [None, "ft", "usa", "scaleDT"] # ft = fine-tuning
    ...
    #
    # Constructs a full file path for saving or loading a trained model.
    # Adjusts the naming convention based on parameters like:
    # - Transfer learning source country.
    # - Whether OHLC or other chart format options are enabled.
    # Returns the full string path for model persistence.
    ...
    def get_exp_name(self):
        ...
        if self.train_size_ratio != 0.7: # Single training period (1993–2000): 70% train / 30% valid
    ...
    #
    # Prepares and returns dictionaries of DataLoaders for training & validation.
    # - Loads dataset(s) based on the experiment's country and years.
    # - Merges datasets if needed and splits by train/validation ratio.
    # - Supports both image-based (CNN2D) and time-series (CNN1D) inputs.
    # Returns a dictionary with 'train' and 'valid' DataLoader entries.
    ...
    def get_train_validate_dataloaders_dict(self):
        ...
        train_dataloader = DataLoader(
            train_dataset,
            batch_size=cf.BATCH_SIZE, # Batch size is a hyperparameter from config.py
            shuffle=True,
    ...
    def get_df_from_dataloader(self, dataloader):
        ...
        df_dtypes = [object, "datetime64[ns]", int, float, float] #
        df_list = []
    ...
    # train_empirical_ensem_model:
    #
    # Trains an ensemble of CNN models (default ensemble size: 5).
    # - Loops through each ensemble member (different random seeds).
    # - Checks if pretrained weights exist; if so, loads them and skips training.
    # - Otherwise, trains each model using train_single_model.
    # - Collects and logs performance metrics: loss, MCC, accuracy.
    # - Saves a CSV and LaTeX summary of the ensemble’s validation results.
    ...
    # evaluate:
    #
    # Evaluates a trained CNN model on a provided DataLoader.
    # - Runs model in evaluation mode (no gradient computation).
    # - Computes and returns:
    #   - Average loss,
    #   - Accuracy,
    #   - Matthews Correlation Coefficient (MCC),
    #   - Diff (imbalance between predicted classes).
    # - Supports label replacement to test model robustness to label shifts.
    ...
    # train_single_model:
    #
    # Handles the full training process for a single CNN model instance.
    # - Loads the appropriate DataLoaders for training/validation.
    # - Initializes optimizer, scheduler, and optionally loads pretrained weights.
    # - Performs epoch-based training with early stopping based on validation loss.
    # - Tracks best performance metrics and saves model checkpoints accordingly.
    # - Logs detailed metrics after training is complete.
    ...
    def train_single_model(self, dataloaders_dict, model_save_path, model_num=None):
        if self.country != "USA" and self.tl is not None:
            ...
            # Loading a pretrained model (useful in transfer learning/fine-tuning scenarios):
            ...
            if self.tl == "usa":
                ...
                for param in model.parameters():    # This is fine-tuning.
                    param.requires_grad = False     # Freezing all layers.
                # Replacing the final classification head (fully-connected layer):
                model.fc = nn.Linear(model.fc.in_features, 2)
                model.fc.apply(cnn_model.init_weights) 
                # Initializing a new optimizer ONLY for the new layer
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    if name[-8:] == "0.weight":
                        ...
                        prev_weight_dict[name] = param.data.clone()
            # Early stopping based on validation loss (p. 3205):
        ...
        # Loading the best weights, after selecting the best model (early stopping):
        model.load_state_dict(best_model)
        best_validate_metrics["model_state_dict"] = model.state_dict().copy()
        # Saving the trained model's weights & biases along with validation metrics:
    ...
    # _ensemble_results:
    #
    # Aggregates predictions across all trained ensemble models.
    # - Each model's up_prob (probability of up tick) is stored.
    # - Averages probabilities to produce the final ensemble output.
    # - Returns a dictionary containing up_prob, stock returns, market caps, etc.
    ...
    def _ensemble_results(self, model_list, dataloader):
        ...
        df_dtypes = [object, "datetime64[ns]", float, float, float] #
        df_list = []
        for batch in dataloader:
    ...
    # calculate_portfolio:
    #
    # Builds and evaluates a portfolio based on ensemble CNN predictions.
    # - Runs the ensemble to get up_prob predictions.
    # - Constructs long-short or directional portfolios using PortfolioManager.
    # - Handles delayed returns (if applicable) for robustness checks.
    # - Saves results (typically as CSV) for further analysis.    
...
# get_exp_obj_by_spec:
#
# Factory function that instantiates an Experiment object with a specific CNN
# architecture and hyperparameter specification.
# - Takes in hyperparameters like dropout rate, number of channels, kernel sizes, etc.
# - Builds the CNN architecture using cnn_model.py.
# - Returns a fully initialized Experiment object, ready to train.
# Useful for automated grid search or architectural experiments.
...
# get_bl_exp_obj:
#
# Factory function for a ``baseline experiment setup''.
# - Hardcodes the standard CNN architecture and parameters as used in the paper.
# - Returns a fully initialized Experiment object for direct use.
# Helps in reproducing baseline results consistently.
...
# run_arch_comparison:
#
# Automates the comparison of different CNN architectures by:
# - Iterating over multiple architecture specs (e.g., dropout, filter sizes).
# - Running experiments in parallel if multiple workers are specified.
# - Collects performance metrics for each architecture for comparison.
# Useful for systematic exploration of CNN design space.
