...
class GenerateStockData(object):
    def __init__(
        self,
        country,                # (str) Country identifier (e.g., 'US', 'CN') used to load appropriate data
        year,                   # (int) The year of data to generate samples for (e.g., 2000)
        window_size,            # (int) Number of daily data points in one input sample (e.g., 60 for 60-day window)
        freq,                   # (str) Label frequency: 'week', 'month', or 'quarter' used for target label computation
        chart_freq=1,           # (int) How often to sample OHLC bars within window (1 = daily, 5 = weekly, etc.)
        ma_lags=None,           # (list of int or None) List of moving average lags to include (e.g., [5, 10, 20])
        volume_bar=False,       # (bool) Whether to include volume bars in the image chart representation
